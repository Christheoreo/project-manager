...
func (r *UsersRepositoryPostgres) Insert(user models.NewUserDto) (id int, err error) {
...
func (r UsersRepositoryPostgres) GetByID(id int) (user models.UserDto, err error) {
	query := `SELECT "id", "first_name", "last_name", "email" FROM "users" WHERE "id" = $1`
	err = r.Pool.QueryRow(context.Background(), query, id).Scan(&user.ID, &user.FirstName, &user.LastName, &user.Email)
	return
}
func (r UsersRepositoryPostgres) GetByEmail(email string) (user models.UserDto, err error) {
...
func (r UsersRepositoryPostgres) GetPassword(authLogin models.AuthLoginDto) (string, error) {
