...
func (r *TagsRepositoryPostgres) Insert(tag dtos.NewTagDto, userId int) (id int, err error) {
...
func (r *TagsRepositoryPostgres) GetById(id int) (tag dtos.TagDto, err error) {
	query := `SELECT id, "name" FROM tags WHERE id = $1`
	err = r.Pool.QueryRow(context.Background(), query, id).Scan(&tag.ID, &tag.Name)
	return
}
func (r *TagsRepositoryPostgres) GetAllForUser(userID int) (tags []dtos.TagDto, err error) {
